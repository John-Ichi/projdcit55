...
function getLicenseInfoUser($license_number) {
    $conn = connect();
    $sql =
        "SELECT tblogininfo.userId, tblicense.name, tblicense.sex, tblicense.address, tblicense.licenseNumber, tblicense.dateRegistered, tblicense.dateRenewed, tblicense.expirationDate, tblicense.status
        FROM tblicense
        LEFT JOIN tblogininfo
        ON tblicense.licenseNumber = tblogininfo.licenseNumber
        WHERE tblicense.licenseNumber='$license_number'";
    $rs = $conn->query($sql);
    ...
    while ($row = $rs->fetch_assoc()) {
        echo "
            <tr>
                ...
                <td>" . date_format(date_create($row['settlementDeadline']),"F j, Y") ."</td>
...
if (isset($_POST['renewLicense']) && $_POST['renewLicense'] == true) { // Extend license expiration
    ...
    if (date("Y-m-d") < $prev_expiry) {
        returnToLicenseActions($serial_number, "License not yet expired!");
        return;
    }
...
if (isset($_POST['fileViolation']) && $_POST['fileViolation'] == true) { // File violation
    $serial_number = $_POST['serial-number'];
    $settlement_deadline = $_POST['settlement-deadline'];
    $minimum = date_create();
    date_add($minimum,date_interval_create_from_date_string("7 days"));
    $minimum_deadline = date_format($minimum,"Y-m-d");

    if ($settlement_deadline < $minimum_deadline) {
        returnToLicenseActions($serial_number,"Minimum 7 days after filing is required!");
        return;
    }
    ...
    $file_violation->bind_param('sssss',$serial_number,$_POST['license-number'],$_POST['violation-committed'],$_POST['alloted-penalty'],$settlement_deadline);
...
if (isset($_POST['deleteLicense']) && $_POST['deleteLicense'] == true) { // Delete license records
    ...
    $check_status = checkStatus($serial_number);
    if ($check_status == true) {
        returnToLicenseActions($serial_number,"License currently suspended/revoked!");
        return;
    }
...
function checkSuspensionRevocationDeadlines() { // Suspend/revoke licenses not resolved on deadlines
    ...
    $sql =
        "SELECT *
        FROM tbviolations
        ORDER BY settlementDeadline ASC";
    $rs = $conn->query($sql);
...
function checkStatus($serial_number) { // Check if suspended/revoked
    $conn = connect();
    $sql =
        "SELECT status
        FROM tblicense
        ...
        while ($row = $rs->fetch_assoc()) {
            echo "
                <tr>
                    ...
                    <td>" . date_format(date_create($row['dateRegistered']),"F j, Y") . "</td>
                    <td>" . ($row['dateRenewed'] == null ? 'No Record' : date_format(date_create($row['dateRenewed']),"F j, Y") ) . "</td>
                    <td>" . date_format(date_create($row['expirationDate']),"F j, Y") . "</td>
                    <td>" . $row['status'] . "</td>
                    <td>
...
function checkDeleted($license_number) {
    $conn = connect();
    $sql =
        "SELECT licenseNumber
        FROM tblicense
